...
MONITOR_COUNT=$(xrandr --current -d :0| grep connected | grep -v disconnected | wc -l)
...
case "$MONITOR_COUNT" in
    "1")
        ...
        ;;
    "2")
        XRANDR_FLAGS="-d :0" $WS home >> /tmp/docklogger 2>&1
        ;;
    "3")
        XRANDR_FLAGS="-d :0" $WS wrk >> /tmp/docklogger 2>&1
        ;;
esac
exit 0